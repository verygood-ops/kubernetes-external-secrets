...
class KubernetesSecrets {
    ...
    async _listAndRefreshSecrets() {
        ...
        let newSecrets = {}
        let kubeSecrets = await kubeNamespace.secrets.get({limit: 100})

        while (true) {
            // Recursively retrieve all secrets
            console.log('SECRETS ::: ', kubeSecrets)
            for (const kubeSecret of kubeSecrets.body.items) {
                newSecrets[kubeSecret.metadata.name] = kubeSecret;
            }

            if (!kubeSecrets.body.metadata.continue) {
                break
            } else {
                kubeSecrets = await kubeNamespace.secrets.get({limit: 100, continue: kubeSecrets.body.metadata.continue})
            }
