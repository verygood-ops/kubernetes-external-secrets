...
describe('Poller', () => {
  ...
  beforeEach(() => {
    ...
    sinon.stub(kubeNamespaceMock.metadata, 'name').get(() => {return 'ns3'})
    kubeNamespaceMock.secrets = sinon.stub().returns(kubeNamespaceMock)
    kubeNamespaceMock.secrets.get = sinon.stub().resolves([fakeInternalSecret])
  ...
  describe('_scheduleNextPoll', () => {
    ...
    it('does not poll when polling is disabled and there is internal secret present', async () => {
      console.log(KubernetesSecrets.secretObservers)
      poller = new Poller({
        intervalMilliseconds: 5000,
        kubeClient: kubeClientMock,
        logger: loggerMock,
