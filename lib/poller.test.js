...
describe('Poller', () => {
  ...
  beforeEach(() => {
    ...
    kubeNamespaceMock.secrets.get = sinon.stub().resolves({body: {items: [fakeInternalSecret]}})
  ...
  describe('_scheduleNextPoll', () => {
    ...
    it('polls for internal secrets when enabled', async () => {
      poller = new Poller({
        intervalMilliseconds: 5000,
        kubeClient: kubeClientMock,
        logger: loggerMock,
        externalSecret: fakeExternalSecret,
        customResourceManifest: fakeCustomResourceManifest,
        pollInternalSecrets: true,
        metrics: metricsMock
      })
      expect(poller._internalSecrets).instanceOf(KubernetesSecrets)
      delete KubernetesSecrets.secretObservers['ns3']

    })

    it('does not poll when polling is disabled and there is internal secret present', async () => {
      poller = new Poller({
        intervalMilliseconds: 5000,
        kubeClient: kubeClientMock,
        logger: loggerMock,
      ...
      await poller._internalSecrets._listAndRefreshSecrets()
      clearTimeout(poller._internalSecrets._timeoutId)
    ...
    it('polls for external secrets when polling is disabled but there is no internal secret present', async () => {

      for (const member in KubernetesSecrets.secretObservers) delete KubernetesSecrets.secretObservers[member];
      poller = new Poller({
        intervalMilliseconds: 5000,
        kubeClient: kubeClientMock,
        logger: loggerMock,
      ...
      kubeNamespaceMock.secrets.get = sinon.stub().resolves({
        body: {
          items: [{
            apiVersion: 'v1',
            kind: 'Secret',
            metadata: {
              // Different than one present after _listAndRefreshSecrets()
              name: 'fakeSecretNameStub'
            }
          }]
        }
      })
      ...
      expect(externalSecretsApiMock.status.get.calledWith()).to.equal(true)
      sinon.assert.called(poller._setNextPoll)
