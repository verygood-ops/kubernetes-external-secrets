...
class Poller {
  ...
  constructor ({
    ...
    this._namespaceObj = this._kubeClient
        .apis[this._customResourceManifest.spec.group]
        .v1.namespaces(this._namespace)

    this._status = this._namespaceObj[this._customResourceManifest.spec.names.plural](this._name).status

    this._internalSecrets = pollInternalSecrets ?
        KubernetesSecrets.getOrCreateSecretObserver({
          namespace: this._namespaceObj,
  ...
  async _scheduleNextPoll () {
    try {
      ...
      if (this._pollingDisabled) {
        ...
        if ((this._internalSecrets === null) || (this._internalSecrets.secretPresent(this._name))) {
          return
        }
